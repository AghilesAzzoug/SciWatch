...
def test_get_summarizer_error_handling():
    ...
    with pytest.raises(ValueError):
        get_summarizer(type="gpt", summarizer_kwargs={"provider": "unknown provider"})
...
def test_azure_get_summarizer():
    ...
    summarizer = get_summarizer(
        type="gpt", summarizer_kwargs={"provider": "azure", "model_name": "random"}
    )
...
def test_openai_get_summarizer():
    ...
    summarizer = get_summarizer(
        type="gpt", summarizer_kwargs={"provider": "openai", "model_name": "random"}
    )
    assert isinstance(summarizer, GPTSummarizer)
...
def test_gpt_summarize(mocker):
    ...
    doc = Document(
        title="Document", content="Content", date=datetime.now(), url="url.fr"
    )
